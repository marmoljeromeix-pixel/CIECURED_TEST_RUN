...
// XAMPP's default MySQL setup: running on this same computer, user
// "root", no password. If your XAMPP MySQL has a different user or
// password, change these three lines to match.
define('DB_HOST', '127.0.0.1');
define('DB_NAME', 'ciecured');
define('DB_USER', 'root');
define('DB_PASS', '');

// Opens (or reuses) the database connection, creating the database
// and its tables the first time this runs if they don't exist yet —
// so there's nothing to set up by hand in phpMyAdmin.
function get_db() {
    static $pdo = null;
    ...
    try {
        // Connect without naming a database yet, so we can create it
        // if this is the very first run.
        $setup = new PDO('mysql:host=' . DB_HOST, DB_USER, DB_PASS, array(
            PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,
        ));
        $setup->exec('CREATE DATABASE IF NOT EXISTS `' . DB_NAME . '` CHARACTER SET utf8mb4');
